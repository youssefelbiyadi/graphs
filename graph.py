...
class Graph:
    ...
    def _are_neighbours(self, vertex: str, other_vertex: str) -> bool:
    ...
    def add_edge(self, vertex: str, other_vertex: str) -> bool:
        if vertex in self.adjacencies_list and other_vertex in self.adjacencies_list:
            if not self._are_neighbours(vertex=vertex, other_vertex=other_vertex):
                self.adjacencies_list[vertex].append(other_vertex)
                self.adjacencies_list[other_vertex].append(vertex)
                return True
        return False

    def remove_vertex_neighbour(self, vertex: str, neighbour: str) -> bool:
        if vertex not in self.adjacencies_list:
            return False
    ...
    def remove_edge(self, vertex: str, other_vertex: str) -> bool:
        if vertex in self.adjacencies_list and other_vertex in self.adjacencies_list:
            self.remove_vertex_neighbour(vertex=vertex, neighbour=other_vertex)
            self.remove_vertex_neighbour(vertex=other_vertex, neighbour=vertex)
            return True
        return False
    ...
    def remove_vertex(self, vertex: str) -> bool:
        if vertex in self.adjacencies_list:
            ...
            for neighbour in vertex_neighbours:
                self.remove_edge(vertex=vertex, other_vertex=neighbour)
